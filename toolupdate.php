...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $queryquan = "SELECT * FROM tools WHERE ID = $tid"; 
    ...
    if($stats == 'Defective'){
        $fetchID = "SELECT * FROM tools WHERE ID = $GID"; 
        ...
        if($GID == $main){
            ...
            $check3 = "UPDATE tools SET Quantity = '$superTotal' WHERE ID = $main";
            $checkres3 = mysqli_query($conn, $check3);
            $deleteborrow = "DELETE FROM tools WHERE ID = $tid";
            $delres = mysqli_query($conn, $deleteborrow);
        }else{
            $check = "UPDATE tools SET Quantity = '$qty', Status = '$status', Name = '$name', Brand = '$brand', Date = '$date' WHERE ID = $tid";
    ...
    if($quantity <= $qty){
        ...
        VALUES('$tid', '$name', '$brand', '$date', '$qty', '$status')";
        $checkres = mysqli_query($conn, $check);

        $check2 = "UPDATE tools SET Quantity = '$total' WHERE ID = $tid";
