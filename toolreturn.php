...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $gid = "SELECT * FROM tools WHERE ID = $did"; 
    ...
    $checkname = "SELECT * FROM tools WHERE Name = '$name' AND (Status = 'Working' OR Status = 'Defective')"; 
    $checkresult = mysqli_query($conn, $checkname);
    if (mysqli_num_rows($checkresult) <= 0) {
        $check = "UPDATE tlog SET TimeOut = '$in' WHERE TID = $did" ;
        $checkres = mysqli_query($conn, $check);

        $up = "UPDATE tools SET Status = 'Working' WHERE ID = $did" ;
        $upres = mysqli_query($conn, $up);
    }else{
    $check = "UPDATE tlog SET TimeOut = '$in' WHERE ID = $logid" ;
    $checkres = mysqli_query($conn, $check);

    $query2 = "SELECT * FROM tlog WHERE ID = $logid"; 
    $rs2 = mysqli_query($conn, $query2);
    $fetch2 = mysqli_fetch_assoc($rs2);
    $mainID = $fetch2['TID'];

    $query3 = "SELECT * FROM tools WHERE ID = $mainID"; 
    ...
    $up = "UPDATE tools SET Quantity = '$newquantity' WHERE ID = $mainID" ;
    $upres = mysqli_query($conn, $up);  

    $deleteborrow = "DELETE FROM tools WHERE ID = $did";
...
if ($checkres) {
    ...
    echo "Tool Returned Successfuly";
    echo '</div>';
