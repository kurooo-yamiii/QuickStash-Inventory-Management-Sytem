...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = "SELECT * FROM tools WHERE Status = '$status'";
    $checkres = mysqli_query($conn, $check);

        
if ($checkres) {
    echo '<table><thead>
    <tr><th class="table-header" scope="col">'. $title .' TOOLS</th></tr>
</thead></table>';
    ...
    if (mysqli_num_rows($checkres)) {
        ...
        while ($rows = mysqli_fetch_assoc($checkres)) {
            ...
            echo '<td>';
            if ($status == 'Working'){
            echo '<a onclick="deleteTool(' . $rows['ID'] . ')" type="button" class="red-button">Delete</a>';
            echo '<a onclick="toolconsUpdate(' . $rows['ID'] . ')" type="button" class="blue-button">Update</a>';
            echo '<a onclick="toolconsBorrow(' . $rows['ID'] . ')" type="button" class="green-button">Borrow</a>';
            }else if ($status == 'Defective') {
                echo '<a onclick="deleteTool(' . $rows['ID'] . ')" type="button" class="red-button">Delete</a>';
                echo '<a onclick="toolconsUpdate(' . $rows['ID'] . ')" type="button" class="blue-button">Update</a>';
                }else {
                   
                    echo '<a onclick="toolreturn(' . $rows['ID'] . ')" type="button" class="violet-button">Return</a>';
                }
            echo '</td>';
            echo '</tr>';
        }
